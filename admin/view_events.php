...
<?php include('includes/db_conn.php'); ?>
    ...
    <section id="main" class="column">		
        <?php
            if($_SESSION['message'] == 'delete_event_success'){
        ...
        <article class="module width_full">
			...
			<tbody> 
                            <?php
                                $no = 0;
                                $q  = mysql_query("SELECT * FROM `events` ORDER BY `date` ASC");
                                while($row = mysql_fetch_array($q)){
                                    ...
                                    echo"
                                        <tr>
                                            ...
                                            <td>{$row['name']}</td>
                                             <td>
                                                <a href='edit_event.php?id={$row['id']}' class='edit'><input type='image'src='images/icn_edit_article.png' title='Edit'></a>
                                                <a href='includes/delete_event_process.php?id={$row['id']}' class='delete'><input type='image'src='images/icn_trash.png' title='Trash'></a>
                                            </td>
                                        </tr>
                                        ";
                                }
